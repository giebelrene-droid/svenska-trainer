...
function initPictureQuiz() {
    ...
    if (!grid) return;
    const EMOJI_COLORS = ['#EDE9FE','#FEF3C7','#D1FAE5','#FEE2E2'];
    grid.innerHTML = pool.map((w, i) => {
        ...
        return `<div onclick="checkPictureQuiz(${i})" id="picCard_${i}" style="cursor:pointer;border-radius:16px;overflow:hidden;border:3px solid var(--border-soft);background:rgba(255,255,255,0.8);transition:border-color 0.2s;">
            <div id="picImg_${i}" style="width:100%;height:110px;background:${EMOJI_COLORS[i]};display:flex;align-items:center;justify-content:center;font-size:2.5rem;">🔄</div>
            <div style="padding:6px 8px;text-align:center;font-weight:800;font-size:0.9rem;color:var(--primary);">${escapeHTML(w[conf.l3])}</div>
        </div>`;
    }).join('');
    pool.forEach((w, i) => {
        const keyword = encodeURIComponent(w[conf.l2] || w[conf.l1]);
        fetch(`https://en.wikipedia.org/api/rest_v1/page/summary/${keyword}`)
            .then(r => r.json())
            .then(data => {
                const imgEl = document.getElementById(`picImg_${i}`);
                if (!imgEl) return;
                if (data.thumbnail && data.thumbnail.source) {
                    imgEl.innerHTML = `<img src="${data.thumbnail.source}" style="width:100%;height:110px;object-fit:cover;display:block;">`;
                } else {
                    imgEl.innerHTML = `<span style="font-size:3rem;">${w[conf.l1]?.charAt(0) || '?'}</span>`;
                }
            })
            .catch(() => {
                const imgEl = document.getElementById(`picImg_${i}`);
                if (imgEl) imgEl.innerHTML = `<span style="font-size:3rem;">${w[conf.l1]?.charAt(0) || '?'}</span>`;
            });
    });
}

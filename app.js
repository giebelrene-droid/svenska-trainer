...
const LIST_PAGE_SIZE = 150; let listRenderLimit = LIST_PAGE_SIZE;
let chatMsgCounter = 0; let chatBusy = false;
let chatRecorder = null, chatStream = null, chatChunks = [], chatAudioCtx = null, chatSilenceRAF = null, chatMaxTimer = null, chatSpeechSeen = false, chatRecMime = '';
...
function speak(text, langKey, rate = 1.0, onDone = null) {
    ...
    if (elevenlabsApiKey) {
        ...
        if (voiceId) { speakElevenLabs(text, voiceId).then(done); return; }
    }
    if (!window.speechSynthesis) { done(); return; }
    const ss = window.speechSynthesis;
    ss.cancel();
    // Small delay after cancel() so mobile browsers don't cut off the new utterance
    setTimeout(() => {
        ss.resume();
        const msg = new SpeechSynthesisUtterance(text.trim());
        msg.lang   = (ALL_LANGS[langKey] && ALL_LANGS[langKey].tts) || 'de-DE';
        ...
        if (voice) msg.voice = voice;
        msg.onend = () => done();
        msg.onerror = (e) => { if (e.error !== 'interrupted') logCustomError('speak', e.error || String(e)); done(); };
        ss.speak(msg);
    }, 80);
}
...
function registerServiceWorker() {
    ...
    // network-first HTML + cache-first versioned assets → fast loads, updates still arrive
...
function populateLangSelects() { 
    ...
    ['selL1','selL2','selL3','liveSrcLang','liveTgtLang','chatSrcLang','chatTgtLang'].forEach(id => { const el = document.getElementById(id); if(!el) return; el.innerHTML = opts; if(id === 'selL1') el.value = conf.l1; if(id === 'selL2') el.value = conf.l2; if(id === 'selL3') el.value = conf.l3; if(id === 'chatTgtLang') el.value = conf.l3; if(id === 'chatSrcLang') el.value = conf.l2; });
}
...
async function photoFromOpenverse(q) {
    try {
        const r = await fetch(`https://api.openverse.org/v1/images/?q=${encodeURIComponent(q)}&category=photograph&page_size=8&mature=false`, { headers: { 'Accept': 'application/json' } });
        if (!r.ok) return null;
        const d = await r.json();
        const res = (d && d.results) ? d.results.filter(x => x.thumbnail || x.url) : [];
        if (!res.length) return null;
        const p = res[Math.floor(Math.random() * Math.min(res.length, 4))];
        return p.thumbnail || p.url;
    } catch { return null; }
}
async function photoFromWikipedia(q) {
    try {
        const r = await fetch(`https://en.wikipedia.org/w/api.php?action=query&generator=search&gsrsearch=${encodeURIComponent(q)}&gsrlimit=3&prop=pageimages&piprop=thumbnail&pithumbsize=400&format=json&origin=*`);
        if (!r.ok) return null;
        const d = await r.json();
...
async function photoFromWikimedia(q) {
    try {
        const r = await fetch(`https://commons.wikimedia.org/w/api.php?action=query&generator=search&gsrsearch=${encodeURIComponent(q)}&gsrnamespace=6&gsrlimit=3&prop=imageinfo&iiprop=url&iiurlwidth=400&format=json&origin=*`);
        if (!r.ok) return null;
        const d = await r.json();
        const pages = (d && d.query && d.query.pages) ? Object.values(d.query.pages) : [];
        for (const p of pages) { const ii = p.imageinfo && p.imageinfo[0]; if (ii && ii.thumburl) return ii.thumburl; }
        return null;
    } catch { return null; }
}
async function fetchRealPhoto(wordDE, wordEN) {
    // Query all sources in PARALLEL, then pick by priority (Openverse > Wikipedia > Wikimedia).
    const q = (wordEN && wordEN.trim()) ? wordEN : wordDE;
    const [ov, wp, wm] = await Promise.all([
        photoFromOpenverse(q).catch(() => null),
        photoFromWikipedia(q).catch(() => null),
        photoFromWikimedia(q).catch(() => null)
    ]);
    if (ov || wp || wm) return ov || wp || wm;
    // Last resort: try the German word on Openverse in case the English term missed
    if (wordEN && wordDE && wordEN !== wordDE) {
        const url = await photoFromOpenverse(wordDE).catch(() => null);
        if (url) return url;
    }
    return null;
}

async function pickThematicWordsWithImages() {
    if (allWords.length < 4) return null;
    const sample = [...allWords].sort(() => Math.random() - 0.5).slice(0, Math.min(allWords.length, 50));
    const wordList = sample.map(w => w[conf.l1]).join(', ');
    const prompt = `Aus dieser deutschen Wortliste: [${wordList}]
Wähle GENAU 4 Wörter, die möglichst LEICHT VERWECHSELBAR sind, damit die Zuordnung SCHWIERIG wird. Bevorzuge in dieser Reihenfolge:
1. Einzahl/Mehrzahl- oder männlich/weiblich-Varianten falls vorhanden (z.B. Junge, Jungen, Mädchen, Mädchen).
2. Wörter aus derselben ENGEN Kategorie, die man verwechseln kann (z.B. verschiedene Obstsorten, verschiedene Zimmer, verschiedene Werkzeuge).
3. Ähnlich aussehende oder klingende Dinge.
Wähle NICHT völlig verschiedene Themen.
Gib für jedes Wort das beste passende Emoji als Fallback.
Antworte NUR in diesem JSON-Format ohne Erklärung:
[{"word":"Apfel","emoji":"🍎"},{"word":"Banane","emoji":"🍌"},{"word":"Orange","emoji":"🍊"},{"word":"Traube","emoji":"🍇"}]`;
    try {
        ...
        const found = parsed.map(item => {
            const w = allWords.find(a => a[conf.l1].toLowerCase() === (item.word||'').toLowerCase());
            return w ? { ...w, _emoji: item.emoji || '❓', _imgSrc: null } : null;
        }).filter(Boolean);
        if (found.length < 4) return null;
        const words = found.slice(0, 4);
        // Fetch real photos in parallel (English term searches best on Openverse)
        const imgResults = await Promise.all(words.map(w => fetchRealPhoto(w[conf.l1], w[conf.l2])));
        imgResults.forEach((src, i) => { if (src) words[i]._imgSrc = src; });
        return words;
    } catch { return null; }
}

async function prefetchNextPicQuiz() {
    try {
        const pool = await pickThematicWordsWithImages();
        if (pool && pool.length >= 4) picQuizNextBuffer = pool;
    } catch { /* silent */ }
}

async function initPictureQuiz() {
    const grid = document.getElementById('picQuizGrid');
    const wordsEl = document.getElementById('picQuizWords');
    const feedbackEl = document.getElementById('picQuizFeedback');
    const wordEl = document.getElementById('picQuizWord');
    const speakBtn = document.getElementById('picQuizSpeakBtn');
    if (!grid) return;
    if (allWords.length < 4) { grid.innerHTML = '<p style="text-align:center;color:#EF4444;font-weight:700;">Bitte mindestens 4 Wörter hinzufügen!</p>'; return; }
    picQuizState.answered = false;
    picQuizState.phase = 'loading';
    if (feedbackEl) feedbackEl.innerHTML = '';
    if (wordsEl) wordsEl.innerHTML = '';
    if (speakBtn) speakBtn.style.display = 'none';
    const COLORS = ['#EDE9FE','#FEF3C7','#D1FAE5','#FEE2E2'];
    grid.innerHTML = COLORS.map((c, i) => `<div id="picCard_${i}" style="position:relative;border-radius:16px;overflow:hidden;border:3px solid var(--border-soft);background:rgba(255,255,255,0.8);transition:all 0.3s;">
        <div id="picImg_${i}" style="width:100%;height:130px;background:${c};display:flex;align-items:center;justify-content:center;font-size:4rem;">⏳</div>
        <div id="picLabel_${i}" style="padding:6px 8px;text-align:center;font-weight:800;font-size:0.85rem;color:var(--primary);min-height:0;"></div>
    </div>`).join('');
    // Use prefetched buffer if available, otherwise fetch now
    let pool = null;
    if (picQuizNextBuffer) {
        pool = picQuizNextBuffer;
        picQuizNextBuffer = null;
        if (wordEl) wordEl.innerHTML = '<span style="font-size:0.9rem;opacity:0.8;">✅ Rätsel bereit!</span>';
    } else {
        if (wordEl) wordEl.innerHTML = '<span style="font-size:0.9rem;opacity:0.8;">🤖 KI lädt Bilder...</span>';
        pool = await pickThematicWordsWithImages();
    }
    if (!pool || pool.length < 4) {
        pool = [...allWords].sort(() => Math.random() - 0.5).slice(0, 4).map(w => ({...w, _emoji: '❓', _imgSrc: null}));
    }
    const correctIdx = Math.floor(Math.random() * pool.length);
    picQuizState.correct = pool[correctIdx];
    picQuizState.options = pool;
    pool.forEach((w, i) => {
        const imgEl = document.getElementById(`picImg_${i}`);
        if (!imgEl) return;
        imgEl.style.background = COLORS[i];
        if (w._imgSrc) {
            imgEl.innerHTML = `<img src="${w._imgSrc}" style="width:100%;height:100%;object-fit:cover;" alt="${escapeHTML(w[conf.l1])}" onerror="this.parentNode.innerHTML='<span style=\\'font-size:5rem;line-height:1;\\'>${w._emoji || '❓'}</span>'">`;
        } else {
            imgEl.innerHTML = `<span style="font-size:5rem;line-height:1;">${w._emoji || '❓'}</span>`;
        }
    });
    picQuizStartLearnPhase();
}

function picQuizStartLearnPhase() {
    // Prefetch next round in background while user is learning
    if (!picQuizNextBuffer) prefetchNextPicQuiz();
    const wordEl = document.getElementById('picQuizWord');
    const feedbackEl = document.getElementById('picQuizFeedback');
    const pool = picQuizState.options;
    if (feedbackEl) feedbackEl.innerHTML = '<span style="font-size:0.85rem;color:var(--text-light);">📖 Lernphase — merke dir die Wörter!</span>';
    pool.forEach((w, i) => {
        const lbl = document.getElementById(`picLabel_${i}`);
        if (lbl) { lbl.style.minHeight = '28px'; lbl.innerHTML = `<span style="color:var(--primary);">${escapeHTML(w[conf.l3])}</span>`; }
        const c = document.getElementById(`picCard_${i}`); if(c) { c.onclick = null; c.style.opacity='0.5'; c.style.transform='scale(0.97)'; }
    });
    function showLearnWord(i) {
        if (i >= pool.length) { setTimeout(picQuizShowReview, 600); return; }
        const w = pool[i];
        pool.forEach((_, j) => { const c = document.getElementById(`picCard_${j}`); if(c) { c.style.border = j===i ? '3px solid #8B5CF6' : '3px solid var(--border-soft)'; c.style.opacity = j===i ? '1' : '0.4'; c.style.transform = j===i ? 'scale(1.05)' : 'scale(0.96)'; } });
        if (wordEl) wordEl.innerHTML = `<span style="font-size:0.8rem;opacity:0.8;">Lerne (${i+1}/4):</span><br><strong style="font-size:1.6rem;">${escapeHTML(w[conf.l3])}</strong><br><span style="font-size:1rem;color:#C4B5FD;">${escapeHTML(w[conf.l1])}</span>`;
        speak(w[conf.l3], conf.l3, 0.75);
        setTimeout(() => showLearnWord(i + 1), 2400);
    }
    showLearnWord(0);
}

// Review screen: all words shown together with speakers, user starts quiz when ready
function picQuizShowReview() {
    picQuizState.phase = 'review';
    const wordEl = document.getElementById('picQuizWord');
    const wordsEl = document.getElementById('picQuizWords');
    const feedbackEl = document.getElementById('picQuizFeedback');
    const pool = picQuizState.options;
    if (wordEl) wordEl.innerHTML = '<span style="font-size:0.95rem;">📖 Präge dir die Wörter ein — tippe ein Bild oder 🔊 zum Anhören</span>';
    pool.forEach((w, i) => {
        const lbl = document.getElementById(`picLabel_${i}`);
        if (lbl) { lbl.style.minHeight = '28px'; lbl.innerHTML = `<span style="color:var(--primary);">${escapeHTML(w[conf.l3])}</span><br><span style="font-size:0.72rem;color:#8B5CF6;">${escapeHTML(w[conf.l1])}</span>`; }
        const c = document.getElementById(`picCard_${i}`);
        if (c) { c.style.opacity = '1'; c.style.transform = 'scale(1)'; c.style.border = '3px solid var(--border-soft)'; c.style.boxShadow = 'none'; c.style.cursor = 'pointer'; c.onclick = () => speak(w[conf.l3], conf.l3, 0.8); }
    });
    if (wordsEl) {
        wordsEl.innerHTML = '';
        const replay = document.createElement('button');
        replay.className = 'action-btn btn-secondary';
        replay.style.cssText = 'margin:0;';
        replay.textContent = '🔁 Alle vorlesen';
        replay.onclick = picQuizSpeakAll;
        const start = document.createElement('button');
        start.className = 'action-btn btn-magic';
        start.style.cssText = 'margin:0;';
        start.textContent = '▶️ Quiz starten';
        start.onclick = picQuizStartQuizPhase;
        wordsEl.appendChild(replay);
        wordsEl.appendChild(start);
    }
    if (feedbackEl) feedbackEl.innerHTML = '';
}

function picQuizSpeakAll() {
    const pool = picQuizState.options || [];
    let i = 0;
    (function next() {
        if (i >= pool.length) return;
        const w = pool[i++];
        speak(w[conf.l3], conf.l3, 0.8);
        setTimeout(next, 1600);
    })();
}

function picQuizSpeakCorrect() {
    if (picQuizState.correct) speak(picQuizState.correct[conf.l3], conf.l3, 0.75);
}

function picQuizStartQuizPhase() {
    picQuizState.answered = false;
    picQuizState.phase = 'quiz';
    const wordEl = document.getElementById('picQuizWord');
    const wordsEl = document.getElementById('picQuizWords');
    const feedbackEl = document.getElementById('picQuizFeedback');
    const speakBtn = document.getElementById('picQuizSpeakBtn');
    const pool = picQuizState.options;
    const correct = picQuizState.correct;
    const correctIdx = pool.indexOf(correct);
    // Highlight the ONE asked image, dim the rest, clear labels
    pool.forEach((w, i) => {
        const lbl = document.getElementById(`picLabel_${i}`);
        if (lbl) { lbl.innerHTML = ''; lbl.style.minHeight = '0'; }
        ...
        if (i === correctIdx) {
            ...
            card.style.boxShadow = '0 0 0 4px rgba(245,158,11,0.35), 0 8px 22px rgba(245,158,11,0.45)';
        } else {
            card.style.border = '3px solid var(--border-soft)';
            card.style.opacity = '0.55';
            card.style.transform = 'scale(0.95)';
            card.style.boxShadow = 'none';
        }
    });
    if (wordEl) wordEl.innerHTML = `<span style="font-size:1rem;">🎯 Welches Wort passt zum leuchtenden Bild?</span>`;
    // Build 4 shuffled word choices, each with its own speaker button.
    // We do NOT speak the correct word automatically — that would make it too easy.
    const order = [...pool].sort(() => Math.random() - 0.5);
    picQuizState.wordButtons = [];
    if (wordsEl) {
        wordsEl.innerHTML = '';
        order.forEach((w) => {
            ...
            btn.style.cssText = 'flex:1;margin:0;padding:14px 6px;font-size:1.05rem;font-weight:800;border-radius:14px;border:2px solid var(--border-soft);background:var(--card-bg,#fff);color:var(--primary);cursor:pointer;';
            btn.textContent = w[conf.l3];
            btn.onclick = () => checkPictureWord(w, btn);
            const spk = document.createElement('button');
            spk.textContent = '🔊';
            spk.style.cssText = 'width:46px;flex-shrink:0;margin:0;border:2px solid var(--border-soft);border-radius:14px;background:rgba(139,92,246,0.12);font-size:1.1rem;cursor:pointer;';
            spk.onclick = (e) => { e.stopPropagation(); speak(w[conf.l3], conf.l3, 0.8); };
            wrap.appendChild(btn); wrap.appendChild(spk);
            wordsEl.appendChild(wrap);
            picQuizState.wordButtons.push({ w, btn });
        });
    }
    if (feedbackEl) feedbackEl.innerHTML = '<span style="font-size:0.8rem;color:var(--text-light);">Ordne das leuchtende Bild dem richtigen Wort zu — 🔊 zum Anhören.</span>';
    if (speakBtn) speakBtn.style.display = 'none';
}

function checkPictureWord(chosen, btnEl) {
    if (picQuizState.answered || picQuizState.phase !== 'quiz') return;
    picQuizState.answered = true;
    const isCorrect = chosen === picQuizState.correct;
    (picQuizState.wordButtons || []).forEach(({ w, btn }) => {
        btn.onclick = null;
        btn.style.cursor = 'default';
        if (w === picQuizState.correct) { btn.style.border = '2px solid #10B981'; btn.style.background = 'rgba(16,185,129,0.15)'; }
    });
    if (!isCorrect && btnEl) { btnEl.style.border = '2px solid #EF4444'; btnEl.style.background = 'rgba(239,68,68,0.15)'; }
    // Now speak the correct word so the user hears the right pronunciation
    picQuizSpeakCorrect();
    const feedbackEl = document.getElementById('picQuizFeedback');
    if (isCorrect) {
        addXP(10); playSound('success');
        if (feedbackEl) feedbackEl.innerHTML = '<span style="color:#10B981;font-weight:900;font-size:1.1rem;">✅ Richtig! +10 XP</span>';
    } else {
        playSound('error');
        if (feedbackEl) feedbackEl.innerHTML = `<span style="color:#EF4444;font-weight:900;font-size:1.1rem;">❌ Falsch — Richtig: <strong>${escapeHTML(picQuizState.correct[conf.l3])}</strong></span>`;
    }
    setTimeout(() => initPictureQuiz(), 2400);
}
...
// --- Tolerant pronunciation matching helpers ---
...
function normWord(s) { return (s || '').toLowerCase().replace(/[.,!?;:()"'`´]/g, '').replace(/\s+/g, ' ').trim(); }
// Compare every recognizer alternative (and each individual word) against the target
function bestPronMatch(alts, target) {
    const t = normWord(target);
    let best = { dist: 99, ok: false };
    (alts || []).forEach(a => {
        const s = normWord(a);
        if (!s) return;
        let dist = levenshtein(s, t);
        s.split(' ').forEach(word => { dist = Math.min(dist, levenshtein(word, t)); });
        if (dist < best.dist) best.dist = dist;
    });
    const thr = t.length <= 4 ? 1 : (t.length <= 7 ? 2 : 3);
    best.ok = best.dist <= thr;
    return best;
}
function evaluateFcPronunciation(alts, target) {
    const box = document.getElementById('fcFeedbackBox');
    const txt = document.getElementById('fcFeedbackText');
    if (box) box.style.display = 'none';
    const m = bestPronMatch(alts, target);
    let feedback;
    if (m.ok) {
        if (m.dist === 0) { feedback = "Perfekt ausgesprochen! 🌟"; addXP(5); }
        else { feedback = "Sehr gut — fast perfekt! 👍"; addXP(3); }
        playSound('success');
    } else {
        feedback = `Nicht ganz getroffen. Ziel: „${target}". Versuch es nochmal! 🔁`;
        playSound('error');
    }
    if (txt) txt.innerText = feedback;
    if (box) { box.style.display = 'block'; setTimeout(() => box.scrollIntoView({ behavior: 'smooth', block: 'nearest' }), 100); }
}
function recordFcSpeech(e) {
    ...
    try { const rec = new SpeechRecognition(); rec.lang = (ALL_LANGS[conf.l3] && ALL_LANGS[conf.l3].tts) ? ALL_LANGS[conf.l3].tts : 'sv-SE'; rec.maxAlternatives = 6; rec.interimResults = false; const btn = document.getElementById('fcMicBtn'); btn.classList.add('mic-active'); const targetWord = fcPool[fcIndex][conf.l3]; document.getElementById('fcFeedbackBox').style.display = 'none'; rec.onresult = (ev) => { const res = ev.results[0]; const alts = []; for (let i = 0; i < res.length; i++) alts.push(res[i].transcript); btn.classList.remove('mic-active'); document.getElementById('fcUserSaid').innerText = alts[0] || '...'; evaluateFcPronunciation(alts, targetWord); }; rec.onerror = (ev) => { btn.classList.remove('mic-active'); if (ev.error === 'not-allowed') showToast("⚠️ Mikrofon-Zugriff blockiert.", "error"); }; rec.onend = () => btn.classList.remove('mic-active'); rec.start(); } catch(err) { document.getElementById('fcMicBtn').classList.remove('mic-active'); }
}
...
function toggleChatTranslation(id) {
    const el = document.getElementById('chatTrans_' + id);
    if (el) el.style.display = (el.style.display === 'none' || !el.style.display) ? 'block' : 'none';
}
function setChatStatus(txt) { const el = document.getElementById('chatStatus'); if (el) el.innerText = txt || ''; }
function blobToBase64(blob) { return new Promise((res) => { const r = new FileReader(); r.onloadend = () => { const s = r.result || ''; res(String(s).split(',')[1] || ''); }; r.onerror = () => res(''); r.readAsDataURL(blob); }); }
function chatPickRecMime() {
    const cands = ['audio/mp4', 'audio/ogg;codecs=opus', 'audio/webm;codecs=opus', 'audio/webm'];
    for (const c of cands) { try { if (window.MediaRecorder && MediaRecorder.isTypeSupported(c)) return c; } catch(e){} }
    return '';
}
// Accurate speech-to-text via Gemini (records real audio, sends it to the model)
async function transcribeWithGemini(base64, mimeType, langName) {
    const keys = geminiApiKey.split(',').map(k => k.trim()).filter(Boolean);
    if (!keys.length || !base64) return null;
    const instruction = `Transcribe this audio EXACTLY as spoken. The spoken language is ${langName}. Return ONLY the plain transcription text — no quotes, no translation, no extra commentary. If there is no clear speech, return exactly: [UNCLEAR]`;
    for (const model of GEMINI_MODELS) {
        for (const key of keys) {
            try {
                const url = `https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent?key=${key}`;
                const body = { contents: [{ parts: [ { text: instruction }, { inlineData: { mimeType: mimeType, data: base64 } } ] }] };
                const resp = await fetch(url, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) });
                if (!resp.ok) continue;
                const d = await resp.json();
                const txt = (d && d.candidates && d.candidates[0] && d.candidates[0].content && d.candidates[0].content.parts || []).map(p => p.text || '').join('').trim();
                if (txt) return txt;
            } catch(e) { /* try next */ }
        }
    }
...
function chatResume() {
    ...
    if (geminiApiKey && geminiApiKey.trim()) chatListenNext();
    else startTutorMic(); // fallback: browser recognition when no Gemini key
}
async function chatListenNext() {
    if (!isChatSessionActive) return;
    chatChunks = []; chatSpeechSeen = false;
    try {
        if (!chatStream) chatStream = await navigator.mediaDevices.getUserMedia({ audio: { echoCancellation: true, noiseSuppression: true, autoGainControl: true } });
    } catch(e) { showToast("⚠️ Mikrofon-Zugriff nötig.", "error"); stopChatSession(); return; }
    chatRecMime = chatPickRecMime();
    try { chatRecorder = chatRecMime ? new MediaRecorder(chatStream, { mimeType: chatRecMime }) : new MediaRecorder(chatStream); }
    catch(e) { try { chatRecorder = new MediaRecorder(chatStream); } catch(e2) { showToast("⚠️ Aufnahme nicht möglich.", "error"); stopChatSession(); return; } }
    chatRecMime = chatRecorder.mimeType || chatRecMime || 'audio/webm';
    chatRecorder.ondataavailable = (e) => { if (e.data && e.data.size > 0) chatChunks.push(e.data); };
    chatRecorder.onstop = () => onChatRecordingStopped();
    chatRecorder.start();
    setChatStatus("🔴 Sprich jetzt … (ich warte, bis du fertig bist)");
    startChatSilenceDetection();
    clearTimeout(chatMaxTimer); chatMaxTimer = setTimeout(() => stopChatRecorder(), 15000); // safety cap
}
function startChatSilenceDetection() {
    try {
        chatAudioCtx = new (window.AudioContext || window.webkitAudioContext)();
        const src = chatAudioCtx.createMediaStreamSource(chatStream);
        const analyser = chatAudioCtx.createAnalyser();
        analyser.fftSize = 2048;
        src.connect(analyser);
        const data = new Uint8Array(analyser.fftSize);
        let silenceStart = Date.now();
        const SIL_MS = 1500, THRESH = 0.018;
        const tick = () => {
            if (!chatRecorder || chatRecorder.state !== 'recording') return;
            analyser.getByteTimeDomainData(data);
            let sum = 0; for (let i = 0; i < data.length; i++) { const v = (data[i] - 128) / 128; sum += v * v; }
            const rms = Math.sqrt(sum / data.length);
            if (rms > THRESH) { chatSpeechSeen = true; silenceStart = Date.now(); }
            else if (chatSpeechSeen && Date.now() - silenceStart > SIL_MS) { stopChatRecorder(); return; }
            chatSilenceRAF = requestAnimationFrame(tick);
        };
        chatSilenceRAF = requestAnimationFrame(tick);
    } catch(e) { /* no analyser → rely on max timer / manual stop */ }
}
function stopChatRecorder() {
    clearTimeout(chatMaxTimer);
    if (chatSilenceRAF) { cancelAnimationFrame(chatSilenceRAF); chatSilenceRAF = null; }
    if (chatAudioCtx) { try { chatAudioCtx.close(); } catch(e){} chatAudioCtx = null; }
    if (chatRecorder && chatRecorder.state === 'recording') { try { chatRecorder.stop(); } catch(e){} }
}
async function onChatRecordingStopped() {
    if (!isChatSessionActive) return;
    const hist = document.getElementById('chatHistory');
    if (!chatSpeechSeen || !chatChunks.length) { setChatStatus("Ich habe nichts gehört – bitte nochmal."); setTimeout(chatResume, 500); return; }
    chatBusy = true;
    setChatStatus("✍️ Ich verstehe dich …");
    document.getElementById('loaderChat').style.display = 'block';
    const blob = new Blob(chatChunks, { type: chatRecMime });
    const base64 = await blobToBase64(blob);
    const srcLang = document.getElementById('chatSrcLang').value;
    let text = await transcribeWithGemini(base64, (chatRecMime || 'audio/webm').split(';')[0], ALL_LANGS[srcLang].name);
    document.getElementById('loaderChat').style.display = 'none';
    text = (text || '').trim();
    const looksBad = !text || text.length < 1 || /\[UNCLEAR\]/i.test(text) || !/[a-zA-Z0-9äöüåÄÖÜÅ]/.test(text);
    if (looksBad) {
        setChatStatus("");
        hist.innerHTML += `<div class="bubble-feedback">🤔 Das habe ich nicht richtig verstanden. Bitte wiederhole es etwas deutlicher.</div>`;
        hist.scrollTop = hist.scrollHeight;
        chatBusy = false;
        setTimeout(chatResume, 500);
        return;
    }
    setChatStatus("");
...
function stopChatSession() {
    isChatSessionActive = false; chatBusy = false;
    try { if (chatRecorder && chatRecorder.state === 'recording') chatRecorder.stop(); } catch(e){}
    if (chatSilenceRAF) { cancelAnimationFrame(chatSilenceRAF); chatSilenceRAF = null; }
    if (chatAudioCtx) { try { chatAudioCtx.close(); } catch(e){} chatAudioCtx = null; }
    if (chatStream) { try { chatStream.getTracks().forEach(t => t.stop()); } catch(e){} chatStream = null; }
    if (chatRec) { try { chatRec.abort(); } catch(e){} }
    if (window.speechSynthesis) window.speechSynthesis.cancel();
    setChatStatus("");
    const b = document.getElementById('btnChatRecord'); if (b) { b.innerText = "🎤 Chat Starten"; b.style.background = "var(--primary-gradient)"; }
}
async function processChatTurn(userText) {
    // chatBusy is already true here; mic is stopped.
    const srcLang = document.getElementById('chatSrcLang').value;
    const tgtLang = document.getElementById('chatTgtLang').value;
    const hist = document.getElementById('chatHistory');
    hist.innerHTML += `<div class="chat-bubble bubble-user">${escapeHTML(userText)}</div>`;
    hist.scrollTop = hist.scrollHeight;
    document.getElementById('loaderChat').style.display = 'block';
    const prompt = `Du bist ein freundlicher, geduldiger Sprachtutor. Der Nutzer spricht ${ALL_LANGS[srcLang].name} und lernt ${ALL_LANGS[tgtLang].name}.
Führe genau diese 3 Schritte aus:
1) KORREKTUR: Prüfe den Satz des Nutzers auf ${ALL_LANGS[srcLang].name}. Wenn er Fehler enthält, gib die korrigierte, natürliche Version und eine sehr kurze Erklärung auf Deutsch. Wenn alles korrekt ist, schreibe genau: "✅ Perfekt!".
2) ANTWORT: Antworte natürlich, freundlich und kurz (maximal 2 Sätze) auf ${ALL_LANGS[tgtLang].name}, um das Gespräch weiterzuführen (z.B. stelle eine kleine Rückfrage).
3) ÜBERSETZUNG: Übersetze deine Antwort aus Schritt 2 wörtlich ins Deutsche.
Antworte NUR in EINER Zeile in exakt diesem Format, getrennt durch |||, ohne Beschriftungen:
KORREKTUR ||| ANTWORT_AUF_${ALL_LANGS[tgtLang].name.toUpperCase()} ||| DEUTSCHE_ÜBERSETZUNG`;
    const r = await callGemini(userText, null, prompt);
    document.getElementById('loaderChat').style.display = 'none';
    if (r) {
        const parts = r.split('|||');
        const corr = (parts[0] || '').trim();
        const reply = (parts[1] || '').trim();
        const trans = (parts[2] || '').trim();
        const id = ++chatMsgCounter;
        let html = '';
        if (corr) html += `<div class="bubble-feedback">📝 ${escapeHTML(corr)}</div>`;
        html += `<div class="chat-bubble bubble-ai">
            <div>${escapeHTML(reply)}</div>
            <div style="margin-top:8px; display:flex; gap:8px; flex-wrap:wrap;">
                <button class="icon-btn" style="padding:6px 12px; font-size:0.85rem; width:auto;" onclick="speak('${safeJS(reply)}','${tgtLang}')">🔊</button>
                <button class="icon-btn" style="padding:6px 12px; font-size:0.85rem; width:auto;" onclick="toggleChatTranslation(${id})">🇩🇪 Übersetzung</button>
            </div>
            <div id="chatTrans_${id}" style="display:none; margin-top:8px; font-size:0.88rem; color:var(--text-light); border-top:1px solid var(--border-soft); padding-top:6px;">${escapeHTML(trans)}</div>
        </div>`;
        hist.innerHTML += html;
        hist.scrollTop = hist.scrollHeight;
        // Re-open the mic only AFTER the tutor finished speaking (no self-conversation).
        speak(reply, tgtLang, 1.0, () => {
            chatBusy = false;
            if (isChatSessionActive) setTimeout(chatResume, 700);
        });
    } else {
        chatBusy = false;
        if (isChatSessionActive) setTimeout(chatResume, 700);
...
// Some engines emit cumulative final segments ("i'm", "i'm very", "i'm very fine"...).
// Merge them: if each segment extends the previous, keep only the longest; otherwise join.
function mergeSpeechSegments(segs) {
    const list = (segs || []).map(s => (s || '').trim()).filter(Boolean);
    if (!list.length) return '';
    let cumulative = true;
    for (let i = 1; i < list.length; i++) {
        if (!list[i].toLowerCase().startsWith(list[i - 1].toLowerCase())) { cumulative = false; break; }
    }
    return cumulative ? list[list.length - 1] : list.join(' ');
}
function startTutorMic() {
    const SpeechRecognition = window.SpeechRecognition || window.webkitSpeechRecognition;
    if (!SpeechRecognition) return showToast("⚠️ Browser unterstützt keine Spracherkennung.", "error");
    const srcLang = document.getElementById('chatSrcLang').value;
    chatRec = new SpeechRecognition();
    chatRec.lang = ALL_LANGS[srcLang].tts;
    // Listen continuously and only stop after a real pause → captures the WHOLE sentence
    chatRec.continuous = true;
    chatRec.interimResults = true;
    chatRec.maxAlternatives = 3;
    let finalSegments = [];
    let silenceTimer = null;
    const PAUSE_MS = 2600; // wait this long after the user stops talking before sending
    const armSilence = () => { clearTimeout(silenceTimer); silenceTimer = setTimeout(() => { try { chatRec.stop(); } catch(e){} }, PAUSE_MS); };
    chatRec.onstart = () => { finalSegments = []; };
    chatRec.onresult = (e) => {
        finalSegments = [];
        for (let i = 0; i < e.results.length; i++) {
            if (e.results[i].isFinal) finalSegments.push(e.results[i][0].transcript);
        }
        // any speech (interim or final) resets the pause timer
        armSilence();
    };
    chatRec.onerror = (e) => { logCustomError("Chat Tutor Mikrofon", e.error); };
    chatRec.onend = () => {
        clearTimeout(silenceTimer);
        const said = mergeSpeechSegments(finalSegments);
        if (said && isChatSessionActive && !chatBusy) {
            chatBusy = true;
            processChatTurn(said);
        } else if (isChatSessionActive && !chatBusy) {
            // nothing understood yet → keep listening
            setTimeout(() => { try { chatRec.start(); } catch(err){} }, 400);
        }
    };
    try { chatRec.start(); } catch(e){}
}
function toggleChatRecord() {
    if (isChatSessionActive) { stopChatSession(); }
    else {
        isChatSessionActive = true; chatBusy = false;
        const b = document.getElementById('btnChatRecord');
        if (b) { b.innerText = "⏹️ Chat beenden"; b.style.background = "#EF4444"; }
        chatResume();
    }
}
function clearChat() { document.getElementById('chatHistory').innerHTML = `<div class="chat-bubble bubble-ai" id="chatWelcomeMsg">Hej! 👋 Sprich mit mir (z.B. auf Englisch). Ich korrigiere dich und antworte auf der Lernsprache. Tippe „🇩🇪 Übersetzung", wenn du etwas nicht verstehst.</div>`; }
...
function debouncedFilterListWords() { clearTimeout(listDebounceTimer); listDebounceTimer = setTimeout(() => { listRenderLimit = LIST_PAGE_SIZE; renderList(); }, 250); }
function renderList() {
    const q = document.getElementById('listSearch') ? document.getElementById('listSearch').value.toLowerCase().trim() : "";
    const filtered = q ? allWords.filter(w => (w[conf.l1] && w[conf.l1].toLowerCase().includes(q)) || (w[conf.l3] && w[conf.l3].toLowerCase().includes(q))) : allWords;
    const cont = document.getElementById('listCont');
    if (!cont) return;
    const shown = filtered.slice(0, listRenderLimit);
    let html = shown.map(w => `<div class="card" style="padding:15px; margin-bottom:10px; cursor:pointer;" onclick="openWordDetail('${w.id}')"><div class="word-item-actions"><button class="icon-btn" onclick="event.stopPropagation();editWord('${w.id}')" style="padding:8px; font-size:1rem;">✎</button><button class="icon-btn danger" onclick="event.stopPropagation();delWord('${w.id}')" style="padding:8px; font-size:1rem;">X</button></div><div style="font-weight:800; color:var(--primary); font-size:1.1rem; padding-right:80px;"><span class="level-dot lvl-${w.level||0}"></span>${escapeHTML(w[conf.l1])}</div><div style="color:var(--text-light); font-size:0.9rem; margin-top:5px; padding-left: 20px;">${escapeHTML(w[conf.l3])} | ${escapeHTML(w[conf.l2])}</div></div>`).join('');
    if (filtered.length > listRenderLimit) {
        html += `<button class="action-btn btn-secondary" onclick="showMoreListWords()" style="margin-top:6px;">⬇️ Mehr laden (${filtered.length - listRenderLimit} weitere)</button>`;
    }
    cont.innerHTML = html;
}
function showMoreListWords() { listRenderLimit += LIST_PAGE_SIZE; renderList(); }
async function delWord(id) { if(!db) return; if(confirm("Löschen?")) { await db.collection('users').doc(currentUser.uid).collection('words_'+currentCollIndex).doc(id).delete(); allWords = allWords.filter(w => w.id !== id); const wc = document.getElementById('wordCount'); if(wc) wc.innerText = allWords.length; renderList(); saveWordsCache(); } }
...
function wordsCacheKey() { return 'trainerWordsCache_' + currentCollIndex; }
function saveWordsCache() {
    try { localStorage.setItem(wordsCacheKey(), JSON.stringify(allWords)); } catch(e) { /* quota — ignore */ }
}
function loadWordsCacheInstant() {
    // Show cached words immediately so the app isn't empty while Firestore loads
    try {
        const raw = localStorage.getItem(wordsCacheKey());
        if (!raw) return false;
        const cached = JSON.parse(raw);
        if (Array.isArray(cached) && cached.length) {
            allWords = cached;
            const wc = document.getElementById('wordCount'); if (wc) wc.innerText = allWords.length;
            dataReady = true;
...
async function refreshData() {
    if(!currentUser || !db) return;
    const hadCache = loadWordsCacheInstant();
    if (!hadCache) dataReady = false;

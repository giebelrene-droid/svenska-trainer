...
async function fetchRealPhoto(wordDE) {
    // Use Unsplash source for real photographs — no API key required
    const sig = Math.floor(Math.random() * 1000);
    const keyword = encodeURIComponent(wordDE);
    const url = `https://source.unsplash.com/400x300/?${keyword}&sig=${sig}`;
    try {
        const resp = await fetch(url, { redirect: 'follow' });
        if (resp.ok && resp.url && !resp.url.includes('source.unsplash.com')) {
            // Got a real redirect to an actual photo
            return resp.url;
        }
        // Fallback: return the source URL directly — browser will follow redirect
        return url;
    } catch {
        return url; // still try — browser may load it fine
    }
}
...
async function initPictureQuiz() {
    ...
    grid.innerHTML = COLORS.map((c, i) => `<div id="picCard_${i}" style="border-radius:16px;overflow:hidden;border:3px solid var(--border-soft);background:rgba(255,255,255,0.8);">
        ...
        <div id="picLabel_${i}" style="padding:6px 8px;text-align:center;font-weight:800;font-size:0.85rem;color:var(--primary);min-height:28px;"></div>
    </div>`).join('');
...
function picQuizStartLearnPhase() {
    ...
    if (feedbackEl) feedbackEl.innerHTML = '<span style="font-size:0.85rem;color:var(--text-light);">📖 Lernphase — merke dir die Bilder!</span>';
    pool.forEach((w, i) => {
        const lbl = document.getElementById(`picLabel_${i}`);
        if (lbl) lbl.innerHTML = `<span style="color:var(--primary);">${escapeHTML(w[conf.l1])}</span><br><span style="font-size:0.75rem;color:#8B5CF6;">${escapeHTML(w[conf.l3])}</span>`;
    ...
    function showLearnWord(i) {
        ...
        if (wordEl) wordEl.innerHTML = `<span style="font-size:0.8rem;opacity:0.8;">Lerne (${i+1}/4):</span><br><strong style="font-size:1.6rem;">${escapeHTML(w[conf.l1])}</strong><br><span style="font-size:1.05rem;color:#C4B5FD;">${escapeHTML(w[conf.l3])}</span>`;
...
function picQuizStartQuizPhase() {
    ...
    picQuizState.options.forEach((_, i) => {
        const lbl = document.getElementById(`picLabel_${i}`);
        if (lbl) lbl.innerHTML = '';
        const c = document.getElementById(`picCard_${i}`);
        if (c) { c.style.border = '3px solid var(--border-soft)'; c.style.opacity = '1'; c.style.transform = 'scale(1)'; c.style.cursor = 'pointer'; c.onclick = () => checkPictureQuiz(i); }
    });
    if (wordEl) wordEl.innerHTML = `<span style="font-size:0.8rem;opacity:0.8;">🎯 Welches Bild passt zu:</span><br><strong style="font-size:1.9rem;">${escapeHTML(correct[conf.l1])}</strong>`;
    if (feedbackEl) feedbackEl.innerHTML = '<span style="font-size:0.8rem;color:var(--text-light);">Tippe auf das richtige Bild!</span>';
    if (speakBtn) { speakBtn.style.display = 'inline-flex'; speakBtn.onclick = () => speak(correct[conf.l3], conf.l3, 0.75); }
    speak(correct[conf.l3], conf.l3, 0.75);
}
...
function checkPictureQuiz(idx) {
    ...
    picQuizState.options.forEach((w, i) => {
        ...
        if (w === picQuizState.correct) card.style.border = '3px solid #10B981';
        else if (i === idx) card.style.border = '3px solid #EF4444';
    });
    const feedbackEl = document.getElementById('picQuizFeedback');
    if (isCorrect) {
        addXP(10);
        ...
        if (feedbackEl) feedbackEl.innerHTML = `<span style="color:#EF4444;font-weight:900;font-size:1.1rem;">❌ Falsch — Richtig: <strong>${escapeHTML(picQuizState.correct[conf.l1])}</strong></span>`;
    }

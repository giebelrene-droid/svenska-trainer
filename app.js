...
async function fetchRealPhoto(wordDE, wordEN) {
    // Try several real-photo sources in order; return the first hit.
    const q = (wordEN && wordEN.trim()) ? wordEN : wordDE;
    const sources = [photoFromOpenverse, photoFromWikipedia, photoFromWikimedia];
    for (const src of sources) {
        const url = await src(q);
        if (url) return url;
    }
    // Last resort: try the German word on Openverse in case English term missed
    if (wordEN && wordDE && wordEN !== wordDE) {
        const url = await photoFromOpenverse(wordDE);
        if (url) return url;
    }
    return null;
}

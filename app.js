...
async function fetchRealPhoto(wordDE, wordEN) {
    // Search real photographs on Openverse (no API key required, CORS-enabled)
    const query = encodeURIComponent((wordEN && wordEN.trim()) ? wordEN : wordDE);
    const url = `https://api.openverse.org/v1/images/?q=${query}&category=photograph&page_size=8&mature=false`;
    try {
        const resp = await fetch(url, { headers: { 'Accept': 'application/json' } });
        if (!resp.ok) return null;
        const data = await resp.json();
        const results = (data && data.results) ? data.results.filter(r => r.thumbnail || r.url) : [];
...
function picQuizStartLearnPhase() {
    ...
    function showLearnWord(i) {
        if (i >= pool.length) { setTimeout(picQuizStartQuizPhase, 700); return; }
        const w = pool[i];

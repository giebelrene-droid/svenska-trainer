...
function startTutorMic() {
    ...
    chatRec.onresult = async (e) => {
        ...
        if (!t) return;                 // ignore empty captures
        chatBusy = true;                // block mic auto-restart until the tutor finished speaking
        try { chatRec.stop(); } catch(err){}
        const hist = document.getElementById('chatHistory');
        hist.innerHTML += `<div class="chat-bubble bubble-user">${escapeHTML(t)}</div>`;
...
KORREKTUR ||| ANTWORT_AUF_${ALL_LANGS[tgtLang].name.toUpperCase()} ||| DEUTSCHE_ÜBERSETZUNG`;
        const r = await callGemini(t, null, prompt);
        ...
        if (r) {
            ...
            // Speak the reply; only re-open the mic AFTER the tutor has finished talking,
            // so the recognizer never hears the tutor's own voice (no self-conversation).
            speak(reply, tgtLang, 1.0, () => {
                chatBusy = false;
                if (isChatSessionActive) setTimeout(startTutorMic, 700);
            });
        } else {
            // No answer from AI — release and let the user try again
            chatBusy = false;
            if (isChatSessionActive) setTimeout(startTutorMic, 700);
        }
    };
    ...
    chatRec.onend = () => {
        // Restart listening ONLY when idle (silence timeout). While busy (thinking/speaking) do nothing.
        if (isChatSessionActive && !chatBusy) { setTimeout(() => { try { chatRec.start(); } catch(err){} }, 400); }
    };
    chatRec.start();
}

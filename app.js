...
async function fetchWikiImage(word) {
    try {
        const url = `https://en.wikipedia.org/w/api.php?action=query&titles=${encodeURIComponent(word)}&prop=pageimages&format=json&pithumbsize=400&origin=*`;
...
async function pickThematicWords() {
    if (allWords.length < 4) return null;
    const sample = [...allWords].sort(() => Math.random() - 0.5).slice(0, Math.min(allWords.length, 40));
    const wordList = sample.map(w => w[conf.l1]).join(', ');
    const prompt = `Aus dieser deutschen Wortliste: [${wordList}]
Wähle GENAU 4 Wörter die thematisch gut zusammenpassen (z.B. alle Lebensmittel, alle Zimmer, alle Tiere, alle Farben, alle Möbel, alle Körperteile, alle Kleidung).
Die 4 Wörter sollen aus der Liste stammen und visuell gut erkennbar sein (konkrete Gegenstände/Lebewesen, keine abstrakten Begriffe).
Antworte NUR mit den 4 deutschen Wörtern exakt wie in der Liste, kommagetrennt, keine Erklärung.`;
    try {
        const result = await callGemini(prompt);
        if (!result) return null;
        const chosen = result.split(',').map(s => s.trim().replace(/["""]/g, '')).filter(Boolean).slice(0, 4);
        const found = chosen.map(name => allWords.find(w => w[conf.l1].toLowerCase() === name.toLowerCase())).filter(Boolean);
        return found.length >= 4 ? found.slice(0, 4) : null;
    } catch { return null; }
}
...
async function initPictureQuiz() {
    ...
    if (wordEl) wordEl.innerHTML = '<span style="font-size:0.9rem;opacity:0.8;">🤖 KI wählt passende Wörter...</span>';
    ...
    grid.innerHTML = COLORS.map((c, i) => `<div id="picCard_${i}" style="border-radius:16px;overflow:hidden;border:3px solid var(--border-soft);background:rgba(255,255,255,0.8);">
        <div id="picImg_${i}" style="width:100%;height:130px;background:${c};display:flex;align-items:center;justify-content:center;font-size:2rem;">⏳</div>
    ...
    const pool = (await pickThematicWords()) || [...allWords].sort(() => Math.random() - 0.5).slice(0, 4);
    const correctIdx = Math.floor(Math.random() * pool.length);
    picQuizState.correct = pool[correctIdx];
    picQuizState.options = pool;
    if (wordEl) wordEl.innerHTML = '<span style="font-size:0.9rem;opacity:0.8;">🖼️ Bilder werden geladen...</span>';
    const imgs = await Promise.all(pool.map(w => fetchWikiImage(w[conf.l2] || w[conf.l1])));
    picQuizState.images = imgs;
    pool.forEach((w, i) => {
        const imgEl = document.getElementById(`picImg_${i}`);
        if (!imgEl) return;
        if (imgs[i]) {
            imgEl.innerHTML = `<img src="${imgs[i]}" style="width:100%;height:130px;object-fit:cover;display:block;">`;
        } else {
            imgEl.style.background = COLORS[i];
            imgEl.innerHTML = `<span style="font-size:2.8rem;">${['🍎','🏠','🐶','🌸','🚗','📚','👕','🌍'][i % 8]}</span>`;
        }
    });

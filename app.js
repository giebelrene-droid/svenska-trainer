...
async function fetchRealPhoto(wordDE) {
    // Generate a real photograph with Google Imagen 3 (uses the Gemini API key)
    const keys = geminiApiKey.split(',').map(k => k.trim()).filter(k => k);
    ...
    try {
        const resp = await fetch(url, {
            ...
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                instances: [{ prompt: `A realistic, high-resolution color photograph of a single ${wordDE}, professional studio photography, plain neutral background, sharp focus, no text, no watermark.` }],
...
[{"word":"Apfel","emoji":"🍎"},{"word":"Banane","emoji":"🍌"},{"word":"Orange","emoji":"🍊"},{"word":"Traube","emoji":"🍇"}]`;
    try {
        ...
        const imgResults = await Promise.all(words.map(w => fetchRealPhoto(w[conf.l1])));

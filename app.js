...
async function initPictureQuiz() {
    ...
    picQuizState.phase = 'learn';
    picQuizState.learnIdx = 0;
    ...
    if (wordEl) wordEl.innerHTML = '<span style="font-size:0.85rem;opacity:0.7;">Bilder werden geladen...</span>';
    if (speakBtn) speakBtn.style.display = 'none';
    grid.innerHTML = pool.map((w, i) => `<div id="picCard_${i}" style="border-radius:16px;overflow:hidden;border:3px solid var(--border-soft);background:rgba(255,255,255,0.8);transition:all 0.25s;position:relative;">
        <div id="picImg_${i}" style="width:100%;height:120px;background:${COLORS[i]};display:flex;align-items:center;justify-content:center;font-size:2rem;">⏳</div>
        <div style="padding:6px 8px;text-align:center;font-weight:800;font-size:0.85rem;color:var(--primary);">${escapeHTML(w[conf.l1])}</div>
    </div>`).join('');
    ...
    pool.forEach((w, i) => {
        ...
        if (!imgEl) return;
        if (imgs[i]) imgEl.innerHTML = `<img src="${imgs[i]}" style="width:100%;height:120px;object-fit:cover;display:block;">`;
        else imgEl.innerHTML = `<div style="font-size:2.5rem;font-weight:900;color:#6D28D9;">${escapeHTML(w[conf.l1].substring(0,3))}</div>`;
    });
...
function picQuizStartLearnPhase() {
    ...
    let idx = 0;
    if (feedbackEl) feedbackEl.innerHTML = '<span style="font-size:0.85rem;color:var(--text-light);">Lerne die Wörter — dann kommt die Frage!</span>';
    pool.forEach((w, i) => { const c = document.getElementById(`picCard_${i}`); if(c) { c.onclick = null; c.style.opacity='0.5'; c.style.transform='scale(0.97)'; } });
    function showLearnWord(i) {
        if (i >= pool.length) { setTimeout(picQuizStartQuizPhase, 600); return; }
        const w = pool[i];
        pool.forEach((_, j) => { const c = document.getElementById(`picCard_${j}`); if(c) { c.style.border = j===i ? '3px solid #8B5CF6' : '3px solid var(--border-soft)'; c.style.opacity = j===i ? '1' : '0.45'; c.style.transform = j===i ? 'scale(1.04)' : 'scale(0.97)'; } });
        if (wordEl) wordEl.innerHTML = `<span style="font-size:1rem;opacity:0.7;">Lerne:</span><br><strong style="font-size:1.6rem;">${escapeHTML(w[conf.l1])}</strong><br><span style="font-size:1.1rem;color:#8B5CF6;">${escapeHTML(w[conf.l3])}</span>`;
        speak(w[conf.l3], conf.l3, 0.75);
        setTimeout(() => showLearnWord(i + 1), 2200);
    }
    showLearnWord(0);
}
...
function picQuizStartQuizPhase() {
    ...
    if (wordEl) wordEl.innerHTML = `<span style="font-size:0.8rem;opacity:0.7;">Welches Bild passt zu:</span><br><strong style="font-size:1.8rem;">${escapeHTML(correct[conf.l1])}</strong>`;
    if (feedbackEl) feedbackEl.innerHTML = '';

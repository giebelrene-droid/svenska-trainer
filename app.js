...
KORREKTUR ||| ANTWORT_AUF_${ALL_LANGS[tgtLang].name.toUpperCase()} ||| DEUTSCHE_ÜBERSETZUNG`;
    ...
    if (r) {
        ...
        speak(reply, tgtLang, 1.0, () => {
            chatBusy = false;
            if (isChatSessionActive) setTimeout(startTutorMic, 700);
        });
    } else {
        chatBusy = false;
        if (isChatSessionActive) setTimeout(startTutorMic, 700);
...
function toggleChatRecord() { if(isChatSessionActive) { isChatSessionActive = false; chatBusy = false; if(chatRec) { try { chatRec.abort(); } catch(e){} } if(window.speechSynthesis) window.speechSynthesis.cancel(); document.getElementById('btnChatRecord').innerText = "🎤 Chat Starten"; document.getElementById('btnChatRecord').style.background = "var(--primary-gradient)"; } else { isChatSessionActive = true; chatBusy = false; document.getElementById('btnChatRecord').innerText = "⏹️ Chat beenden"; document.getElementById('btnChatRecord').style.background = "#EF4444"; startTutorMic(); } }

...
async function generateGeminiImage(wordDE) {
    const keys = geminiApiKey.split(',').map(k => k.trim()).filter(k => k);
    ...
    try {
        const resp = await fetch(url, {
            ...
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                contents: [{ parts: [{ text: `Professional stock photo of: ${wordDE}. Clean white background, photorealistic, high quality, no text, no watermarks.` }] }],
    ...
    } catch { /* ignore, fall back to emoji */ }
    return null;
}
...
[{"word":"Apfel","emoji":"🍎"},{"word":"Banane","emoji":"🍌"},{"word":"Orange","emoji":"🍊"},{"word":"Traube","emoji":"🍇"}]`;
    try {
        ...
        const imgResults = await Promise.all(words.map(w => generateGeminiImage(w[conf.l1])));
...
async function initPictureQuiz() {
    ...
    pool.forEach((w, i) => {
        ...
        if (w._imgSrc) {
            imgEl.innerHTML = `<img src="${w._imgSrc}" style="width:100%;height:100%;object-fit:cover;" alt="${escapeHTML(w[conf.l1])}">`;
        } else {

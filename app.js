...
async function pickThematicWordsWithEmoji() {
...
Gib für jedes Wort das beste passende Emoji.
...
[{"word":"Apfel","emoji":"🍎"},{"word":"Banane","emoji":"🍌"},{"word":"Orange","emoji":"🍊"},{"word":"Traube","emoji":"🍇"}]`;
    try {
        ...
        const found = parsed.map(item => {
            ...
            return w ? { ...w, _emoji: item.emoji || '❓' } : null;
        }).filter(Boolean);
...
async function initPictureQuiz() {
    ...
    const pool = (await pickThematicWordsWithEmoji()) || [...allWords].sort(() => Math.random() - 0.5).slice(0, 4).map(w => ({...w, _emoji: '❓'}));
    ...
    pool.forEach((w, i) => {
        ...
        if (imgEl) { imgEl.style.background = COLORS[i]; imgEl.innerHTML = `<span style="font-size:5rem;line-height:1;">${w._emoji || '❓'}</span>`; }
    });

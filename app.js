...
Wähle GENAU 4 Wörter die alle zum SELBEN Themenbereich gehören (z.B. nur Obst, nur Zimmer, nur Tiere, nur Kleidung, nur Farben, nur Körperteile, nur Fahrzeuge).
...
function picQuizStartQuizPhase() {
    ...
    pool.forEach((w, i) => {
        ...
        if (i === correctIdx) {
            ...
            // Small speaker overlay on the highlighted image
            if (!document.getElementById('picImgSpeak')) {
                const sp = document.createElement('button');
                sp.id = 'picImgSpeak';
                sp.innerHTML = '🔊';
                sp.style.cssText = 'position:absolute;bottom:6px;right:6px;background:rgba(0,0,0,0.55);color:white;border:none;border-radius:50%;width:34px;height:34px;font-size:1rem;cursor:pointer;display:flex;align-items:center;justify-content:center;';
                sp.onclick = (e) => { e.stopPropagation(); picQuizSpeakCorrect(); };
                card.appendChild(sp);
            }
        } else {
    ...
    if (wordEl) wordEl.innerHTML = `<span style="font-size:1rem;">🎯 Welches Wort passt zum markierten Bild?</span>`;
    // Build 4 shuffled word buttons (Swedish)
    ...
    if (wordsEl) {
        ...
        order.forEach((w) => {
            ...
            btn.style.cssText = 'padding:14px 8px;font-size:1.05rem;font-weight:800;border-radius:14px;border:2px solid var(--border-soft);background:var(--card-bg,#fff);color:var(--primary);cursor:pointer;';
            btn.textContent = w[conf.l3];
            btn.onclick = () => checkPictureWord(w, btn);
            wordsEl.appendChild(btn);
        });
    }
    if (feedbackEl) feedbackEl.innerHTML = '<span style="font-size:0.8rem;color:var(--text-light);">Tippe auf das richtige Wort!</span>';
    if (speakBtn) { speakBtn.style.display = 'inline-flex'; speakBtn.onclick = picQuizSpeakCorrect; }
    picQuizSpeakCorrect();
}
...
function checkPictureWord(chosen, btnEl) {
    ...
    const wordsEl = document.getElementById('picQuizWords');
    if (wordsEl) {
        Array.from(wordsEl.children).forEach(b => {
            b.onclick = null;
            b.style.cursor = 'default';
            if (b.textContent === picQuizState.correct[conf.l3]) { b.style.border = '2px solid #10B981'; b.style.background = 'rgba(16,185,129,0.15)'; }
        });
    }
